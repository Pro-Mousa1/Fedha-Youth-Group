...
public class MonthlyRepayment extends JFrame {
    public MonthlyRepayment() {
        ...
        JLabel loanRepaymentInstallmentsLabel = new JLabel("Loan Repayment Amount:");
        loanRepaymentInstallmentsLabel.setBounds(150, 140, 250, 25);
        loanRepaymentInstallmentsLabel.setForeground(Color.YELLOW);
        panel.add(loanRepaymentInstallmentsLabel);
        JTextField loanRepaymentInstallmentsField = new JTextField(15);
        loanRepaymentInstallmentsField.setBounds(150, 165, 300, 25);
        loanRepaymentInstallmentsField.setForeground(Color.BLACK);
        loanRepaymentInstallmentsField.setBackground(Color.WHITE);
        panel.add(loanRepaymentInstallmentsField);
                    ...
                    if (!memberId.isEmpty()) {
                        try {
                            ...
                            if (outstandingLoan > 0) {
                                ...
                                loanRepaymentInstallmentsField.setEnabled(true);
                            } else {
                                ...
                                loanRepaymentInstallmentsField.setEnabled(false);
        ...
        btnSubmit.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String memberId = memberIdField.getText();
                String loanRepaymentAmountStr = loanRepaymentInstallmentsField.getText();
                ...
                try {
                    double loanRepaymentAmount = Double.parseDouble(loanRepaymentAmountStr);
                    double newLoanBalance = loanBorrowed - loanRepaymentAmount;
                    ...
                    FedhaDatabase.insertMonthlyRepayment(memberId, loanBorrowed, loanRepaymentAmount, newLoanBalance, loanRepaymentDate);

                    // Update the loan amount in the database
                    FedhaDatabase.updateLoanAmount(memberId, loanRepaymentAmount);
                    ...
                    ex.printStackTrace();
                    JOptionPane.showMessageDialog(MonthlyRepayment.this, "An error occurred while updating the loan amount.");
                } catch (Exception ex) {
                    ex.printStackTrace();
